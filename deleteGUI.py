...
def deleteCam(filepath):
    ...
    width = 420
    height = 350
    screen_width = droot.winfo_screenwidth()
    screen_height = droot.winfo_screenheight()
    x = (screen_width/2) - (width/2)
    y = (screen_height/2) - (height/2)
    droot.geometry("%dx%d+%d+%d" % (width, height, x, y))
    
    vscrollbar = Scrollbar(droot,width=15)
